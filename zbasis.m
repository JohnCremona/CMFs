...
intrinsic OptimizedOrderBasis(KPoly::SeqEnum,ZSeq::SeqEnum[SeqEnum]:PolredabsPoly:=[]) -> 
...
{ Given the coefficients of an irreducible monic polynomial f in Q[x] and a sequence of algebraic integers in the number field Q[x]/(f(x)) specified in terms of the power basis, returns:
    ...
    if #PolredabsPoly eq 0 then
        ...
        bestpoly := R!PolredabsPoly; ispolredabs:= true;
        Kbest := NumberField(bestpoly);
        assert Degree(Kbest) eq Degree(K);
        r := Roots(ChangeRing(R!KPoly,Kbest));
        assert #r gt 0;
        iotabest := hom<K->Kbest | r[1][1]>;
    end if;
