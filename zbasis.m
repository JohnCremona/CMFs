...
intrinsic OptimizedOrderBasis(KPoly::SeqEnum,ZSeq::SeqEnum[SeqEnum]:KBestPoly:=[],KBestIsPolredabs:=false,KDisc:=0,Verbose:=true) -> 
...
{ Given the coefficients of an irreducible monic polynomial f in Q[x] and a sequence of algebraic integers in the number field Q[x]/(f(x)) specified in terms of the power basis, returns:
    ...
    while cnt lt mcnt do
        while cnt lt mcnt and n lt #ZSeq do n+:=1; if not KBest!Eltseq(Z[n]) in Rationals() then cnt +:= 1; end if; end while;
        assert cnt eq mcnt;
        try
            time O := Order([KBest | KBest!Eltseq(ZBest[i]) : i in [1..n]]);
            OBasis := Basis(O);
    ...
    if Verbose then printf "%.3o secs, generated by %o elements\n", Cputime()-t, mcnt; end if;
    ...
    if Verbose then printf "Done, total time %.3os\n", Cputime()-start; end if;
    return Eltseq(MinimalPolynomial(KBest.1)), OBestBasis, [[r[i]:i in [1..#OBestBasis]]:r in Rows(ZOE)], Integers()!Oindex, KDisc, KBestIsPolredabs, mcnt; 
end intrinsic;
