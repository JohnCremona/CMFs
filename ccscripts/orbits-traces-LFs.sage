...
def do(level, weight, lfun_filename = None, instances_filename = None, hecke_filename = None, traces_filename = None, only_traces = False):
    ...
    #        print "key = %s" % (key,)
            ...
            assert abs(row[schema_lf_dict['sign_arg']] + row_conj[schema_lf_dict['sign_arg']] % 1) < 1e-10, "%s  + %s  = %s" % (row[schema_lf_dict['sign_arg']], row_conj[schema_lf_dict['sign_arg']], row[schema_lf_dict['sign_arg']]+ row_conj[schema_lf_dict['sign_arg']])
